# frozen_string_literal: true

require_relative 'models/book_delete_schema'
require_relative 'models/book_list'
require_relative 'models/book_form_schema'
require_relative 'models/stationery_form_schema'
require_relative 'models/book'
require_relative 'models/stationery'
require_relative 'models/stationery_list'
require_relative 'models/dry_result_forme_wrapper'
require_relative 'models/store'
require_relative 'models/book_filter_form_schema'

require_relative 'models/list_form_schema'
require_relative 'models/shop_list'
require_relative 'models/stationery_add_schema'
require_relative 'models/shop_lists'
require_relative 'models/list_pay_schema'
